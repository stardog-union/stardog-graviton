...
echo '${zk_conf}' > /usr/local/zookeeper-3.5.9/conf/zoo.cfg
echo ${index} > /var/zkdata/myid

localip=`curl http://169.254.169.254/latest/meta-data/local-ipv4`
num=`cat /var/zkdata/myid`

elb_name=`grep server.$num /usr/local/zookeeper-3.5.9/conf/zoo.cfg | sed 's/.*=//' | sed 's/:.*//'`
...
/usr/local/zookeeper-3.5.9/bin/zkServer.sh start
